...
export default function Game() {
    const [boardSize, setBoardSize] = useState<GameState['boardSize']>(6) // 棋盘大小
    const [winLength, setWinLength] = useState<GameState['winLength']>(3) // 连线长度
    ...
    return (
        <div className="game">
            ...
            <div className="game-info">
                {/* 添加排序按钮 */}
                <button onClick={toggleSortOrder}>{isAscending ? '正序' : '倒序'}</button>
                {/* 显示排序历史记录*/}
                <ol>{sortedMoves}</ol>
            </div>
        </div>
